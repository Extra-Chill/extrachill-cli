...
namespace {
	$qr_code_test_wp_bootstrapped = function_exists( 'get_user_by' );

	use ExtraChill\CLI\Commands\Tools\QRCodeCommand;

	if ( ! $qr_code_test_wp_bootstrapped ) {
		...
		// instead: `php tests/QRCodeCommandTest.php` (see homeboy.json self_checks).
