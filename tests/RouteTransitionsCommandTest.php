...
namespace {
	$route_transitions_command_test_wp_bootstrapped = function_exists( 'get_user_by' );

	if ( ! $route_transitions_command_test_wp_bootstrapped ) {
		// Real WordPress (or the WP Codebox managed PHPUnit harness) is
		// already bootstrapped and this file's global function/class doubles
		// would collide with core, so skip the whole file. Run it standalone
		// instead: `php tests/RouteTransitionsCommandTest.php` (see homeboy.json self_checks).
